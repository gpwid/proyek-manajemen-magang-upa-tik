...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-3">
        ...
        <div class="d-flex flex-wrap gap-2">
            <a href="{{ route('admin.permohonan.edit', $permohonan->id) }}" class="btn btn-primary">
    ...
    @php
        ...
        $hasPermohonan   = filled($permohonan->file_permohonan)
                           && Storage::disk('public')->exists($permohonan->file_permohonan);
        $hasSuratBalasan = filled($permohonan->file_suratbalasan)
                           && Storage::disk('public')->exists($permohonan->file_suratbalasan);
    @endphp
            ...
            <div class="card shadow-sm border-0 mb-4">
                ...
                <div class="card-body p-4">
                    ...
                    <style>
                        ...
                        .kv{border:1px solid #eef0f2;border-radius:12px;padding:12px 14px;background:#fff}
                        .kv .label{font-size:.825rem;color:#6c757d;margin:0}
                        .kv .value{font-weight:700;margin:2px 0 0 0}
                        .note-block{grid-column:1/-1;border:1px dashed #d9dee3;background:#f8f9fa;border-radius:12px;padding:14px}
                        .note-title{font-size:.825rem;color:#6c757d;margin-bottom:6px}
                        .note-body{white-space:pre-wrap;line-height:1.55}
                        .note-empty{color:#95a5b0;font-style:italic}
                        .section-head{display:flex;align-items:center;justify-content:space-between;background:#0d6efd;color:#fff;border-radius:10px 10px 0 0;padding:10px 14px}
                        .section-body{border:1px solid #e7ebef;border-top:0;border-radius:0 0 10px 10px}
                    </style>
                    ...
                    <div class="info-grid">
                        ...
                        <div class="kv">
                            ...
                            <p class="value">{{ $permohonan->institute->nama_instansi ?? '-' }}</p>
                        </div>

                        <div class="kv">
                            <p class="label">Jenis Magang</p>
                            <p class="value">{{ $permohonan->jenis_magang }}</p>
                        </div>

                        <div class="kv">
                            <p class="label">No. Surat</p>
                            <p class="value">{{ $permohonan->no_surat }}</p>
                        </div>

                        <div class="kv">
                            <p class="label">Tanggal Surat Masuk</p>
                            <p class="value">{{ optional($permohonan->tgl_suratmasuk)->format('d-m-Y') ?? '-' }}</p>
                        </div>

                        <div class="kv">
                            <p class="label">Tanggal Surat dibuat</p>
                            <p class="value">{{ optional($permohonan->tgl_surat)->format('d-m-Y') ?? '-' }}</p>
                        </div>

                        <div class="kv">
                            <p class="label">Tanggal Mulai Magang</p>
                            <p class="value">{{ optional($permohonan->tgl_mulai)->format('d-m-Y') ?? '-' }}</p>
                        </div>

                        <div class="kv">
                            <p class="label">Tanggal Selesai Magang</p>
                            <p class="value">{{ optional($permohonan->tgl_selesai)->format('d-m-Y') ?? '-' }}</p>
                        </div>
                        ...
                        <div class="kv">
                            ...
                            <p class="value">{{ $permohonan->pembimbing_sekolah ?? '-' }}</p>
                        </div>

                        <div class="kv">
                            <p class="label">Kontak Pembimbing</p>
                            <p class="value">{{ $permohonan->kontak_pembimbing ?? '-' }}</p>
                        </div>

                        {{-- Catatan sebagai blok teks penuh lebar --}}
                        <div class="note-block">
                            <div class="note-title">Catatan</div>
                            @if (filled($permohonan->catatan))
                                <div class="note-body">{{ $permohonan->catatan }}</div>
                            @else
                    ...
                    <div class="mt-4">
                        <div class="section-head">
                            <h5 class="mb-0"><i class="fa-solid fa-users me-2"></i> Peserta pada Surat Permohonan</h5>
                            <span class="badge bg-light text-dark">Total: {{ $permohonan->participants->count() }}</span>
                        </div>
                    ...
                    <div class="mt-4">
                            ...
                            <div>
                                @if ($hasPermohonan)
                                    <a download class="btn btn-success btn-sm"
                                       href="{{ Storage::url($permohonan->file_permohonan) }}">
                        ...
                        <div class="section-body">
                            <div class="ratio ratio-16x9">
                                @if ($hasPermohonan)
                                    <iframe src="{{ Storage::url($permohonan->file_permohonan) }}" title="PDF Permohonan"
                    ...
                    <div class="mt-4">
                        <div class="section-head">
                            ...
                            <div>
                                @if ($hasSuratBalasan)
                                    <a download class="btn btn-success btn-sm"
                                       href="{{ Storage::url($permohonan->file_suratbalasan) }}">
                                        <i class="fa-solid fa-file-arrow-down"></i> Unduh
                                    </a>
                                @else
                                    <span class="badge bg-secondary">Belum diunggah</span>
                        ...
                        <div class="section-body">
                            <div class="ratio ratio-16x9">
                                @if ($hasSuratBalasan)
                                    <iframe src="{{ Storage::url($permohonan->file_suratbalasan) }}" title="PDF Surat Balasan"
                                            style="border:0;width:100%;height:70vh"></iframe>
                                @else
                                    <div class="d-flex align-items-center justify-content-center" style="height:70vh;">
                                        <div class="text-center text-muted">
